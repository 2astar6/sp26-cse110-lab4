...
2. Line 13 will output the final value of `discountedPrice` after the loop ends. The variable is still reachable because it is declared as `var`, even if it is first-declared inside the loop block.
...
4. Line 16 will return the `discounted` array object, `[50, 100, 150]`,  because the `var`-declared variable is accessible from any part of the function.
...
8. Line 16 (this function's end) will return the array object held by the `discounted` variable, `[50, 100, 150]`. Being declared with `let` at the beginning of the function, its block-scope is the entire function, so it can be accessed from anywhere after it inside the function.
9. Line 11 will throw an error because the `i` variable argument to the `console.log()` function is `let`-declared and thus only scoped for the block that it is used in - within the `for` loop, being declared in the loop initialization, update, and increment composite step.
10. Line 12 will output `3`, the length of the `prices` array object passed as an argument to the function, to the console. Being declared as a `const` at the start of the function, it is accessible/scoped to anywhere inside the function after where it is declared. Since it is a `const` it is known to not change and the `for` loop logic for it also does not try to assign something to it, which would throw an error.
11. Line 14 will return the array object, `[50, 100, 150]`, because the `for` loop can update the array even as a `const`. Having a non-primitive object treated as a `const` means that its reference cannot be changed - not that it is immutable. 
12. Notations
